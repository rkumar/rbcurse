...
class Transactions < Datasource
  ...
  def form_populate_proc 
    qfields = qform.fields
...
class TransV < Application

  attr_reader :table_form

  def initialize(unused=nil)
  ...
  def run
    ...
    begin
      ...
      @qfields = fields
      @qapp = QueryApplication.new(@qfields, self) do |app|
        @qform = app.form
        qform_win_rows = 4
        qform_win_cols = 0 # default of ncurses
        qform_win_starty = 1
        qform_win_startx = 0
        @qform_win, @qform_panel = app.create_window(qform_win_rows, 
                                                     qform_win_cols, 
                                                     qform_win_starty,
                                                     qform_win_startx)
        ...
        @fields = TableApplication.create_table_fields(@rows_to_show,
                                                       fieldlen, 
                                                       att_hash={})
        TableApplication.new(@fields, nil, @datasource) do |tapp|

          @table_form = tapp.form
          @datasource.main = tapp
          app.main = tapp
          app.form.main = tapp
          #  @tapp.main = @tapp
          #  @tapp.form.main = @tapp
